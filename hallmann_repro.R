...
# Old model
z_val <- unlist(map(1:nrow(data), ~ sum(z(unlist(ints[.x]), "old", "base"))))
z_val_y <- unlist(map(1:length(ints_y), ~ sum(z_val[unlist(ints_y[.x])])))

# 2.5 % credible interval
z_val_lo <- unlist(map(1:nrow(data), ~ sum(z(unlist(ints[.x]), "old", "low"))))
z_val_y_lo <- unlist(map(1:length(ints_y), ~ sum(z_val_lo[unlist(ints_y[.x])])))

# 97.5 % credible interval
z_val_hi <- unlist(map(1:nrow(data), ~ sum(z(unlist(ints[.x]), "old", "high"))))
z_val_y_hi <- unlist(map(1:length(ints_y), ~ sum(z_val_hi[unlist(ints_y[.x])])))

# New model
new_z_val <- unlist(map(1:nrow(new_data), ~ sum(z(unlist(new_ints[.x]), "new", "base"))))
new_z_val_y <- unlist(map(1:length(new_ints_y), ~ sum(new_z_val[unlist(new_ints_y[.x])])))

# 2.5 % credible interval
new_z_val_lo <- unlist(map(1:nrow(new_data), ~ sum(z(unlist(new_ints[.x]), "new", "low"))))
new_z_val_y_lo <- unlist(map(1:length(new_ints_y), ~ sum(new_z_val_lo[unlist(new_ints_y[.x])])))

# 97.5 % credible interval
new_z_val_hi <- unlist(map(1:nrow(new_data), ~ sum(z(unlist(new_ints[.x]), "new", "high"))))
new_z_val_y_hi <- unlist(map(1:length(new_ints_y), ~ sum(new_z_val_hi[unlist(new_ints_y[.x])])))

m_bio_df <- data.frame("year" = 1:26, "m_bio" = NA, "m_bio_lo" = NA,
                           "m_bio_hi" = NA) # Create initial df

new_m_bio_df <- data.frame("year" = 1:26, "m_bio" = NA, "m_bio_lo" = NA,
                       "m_bio_hi" = NA) # Create initial df
...
fill <- function(model, cred) {
  ...
  data[1:7, coln] <<- z_val_y[1:7]
  data[9, coln] <<- z_val_y[8]
  data[11:13, coln] <<- z_val_y[9:11]
  data[15:26, coln] <<- z_val_y[12:23]

  data[8, coln] <<- data[7, coln]
  data[10, coln] <<- data[9, coln]
  data[14, coln] <<- data[13, coln]
} # helper function
